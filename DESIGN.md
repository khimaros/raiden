...
file-path mapping (raid-stripe geometry + dmsetup offsets + debugfs).
...
any survivor. each esp has a stable per-slot mount (`/boot/efi1`../boot/efiN`)
in fstab by uuid; `/boot/efi` is a relative symlink to the active primary
(`efi1` by default). the primary mounts `auto,nofail`; the mirrors are `noauto`
and stay cold, mounted only briefly when the grub.d hook
(`90_copy_to_efi_mirrors`, src/stack/mod.rs) resyncs them from the primary on
each update-grub. keeping the mirrors cold limits corruption exposure to the one
mounted esp. a dead primary is failed over by re-pointing the symlink at a
survivor (`ln -sfn efiK /boot/efi`) -- no fstab edit or uuid lookup.

`replace` preserves each rebuilt disk's identifying uuids so the baked
fstab/crypttab entries stay valid: the esp is recreated with its original vfat
uuid (read from `/etc/fstab`, applied via `mkfs.msdos -i`), and the luks header
is re-stamped with its original uuid (read from `/etc/crypttab`, applied via
`cryptsetup luksUUID` after `luksFormat`). without the latter a reboot after a
...
correct since they are identical. the non-primary copies mount `noauto` at
`/boot.mirrorN` (siblings of `/boot`, not nested, so the sync never recurses into
the esp mounts under `/boot`). they are addressed in fstab by DEVICE, not uuid,
so the sync writes each physical disk rather than whatever the shared uuid
resolves to first.
...
src/
  ...
  ops.rs        rescue, replace, remove, close, scrub, status (compose phases)
  badblocks.rs  md read-error to file-path mapping (status)
  stack/
